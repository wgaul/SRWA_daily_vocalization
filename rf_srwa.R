...
if(refit_rf_models) {
  ...
  write_rds(rf_binCV_01, file = "./saved_objects/rf_binCV_01.rds")
} else rf_binCV_01 <- read_rds(file = "./saved_objects/rf_binCV_01.rds")
...
if(refit_rf_models) {
  rf_binCV_h0 <- lapply(locs, function(holdout_loc, dat, pred_names, mtry, 
    ...
    test_dat$pred_rf_binCVh0 <-  predict(
      mod, 
      newdata = data.frame(test_dat[, colnames(test_dat) %in% pred_names]), 
      type = "prob")[, 2]
  ...
  standard_dat = standard_dat_binned)
  write_rds(rf_binCV_h0, file = "./saved_objects/rf_binCV_h0.rds")
} else rf_binCV_h0 <- read_rds(file = "./saved_objects/rf_binCV_h0.rds")
...
if(refit_rf_models) {
  rf_boot_bin_cv01 <- lapply(
    ...
    function(x, pred_names, mtry, locs, 
             standard_dat_binned) {
      ## fit location block cross-validated models.  
      ## Each CV fold has one location
      ## removed, so the number of folds is equal to the number of locations.
      ...
      preds_standard_data <- bind_rows(lapply(mods, 
                                              function(x) x$standard_preds))
  ...
  write_rds(rf_boot_bin_cv01, file = "./saved_objects/rf_boot_bin_cv01.rds")
} else rf_boot_bin_cv01 <- read_rds(
  file = "./saved_objects/rf_boot_bin_cv01.rds")
...
if(refit_rf_models) {
  rf_boot_bin_cvh0 <- lapply(
    ...
    standard_dat_binned = standard_dat_binned)
  write_rds(rf_boot_bin_cvh0, file = "./saved_objects/rf_boot_bin_cvh0.rds")
} else rf_boot_bin_cvh0 <- read_rds(
  file = "./saved_objects/rf_boot_bin_cvh0.rds")
#### end bootstrapping ----------------------
